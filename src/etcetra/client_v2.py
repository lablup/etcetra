...
The difference between `client` and `client_v2` is that there is no encoding or decoding.
"""
...
from .client import (
    ...
    EtcdAuthStreamStreamInterceptor,
)
...
class EtcdClient:
    ...
    def with_lock(
        ...
        return EtcdConnectionManager(
            self._build_connector_protocol(),
            lock_option=lock_opt,
        )
...
class EtcdCommunicator:
    ...
    async def get_range(
        ...
        ret: MutableMapping[bytes, bytes] = OrderedDict()
    ...
    async def delete(
        ...
        """
        stub = rpc_pb2_grpc.KVStub(self.channel)
        response = await stub.DeleteRange(rpc_pb2.DeleteRangeRequest(key=key))
    ...
    async def keys_range(
        ...
        return [x.key for x in response.kvs]
            ...
            while True:
                ...
                for event in response.events:
                    ...
                    if prev_kv and event.prev_kv is not None and event.prev_kv.value is not None:
                        prev_value = event.prev_kv.value
                    else:
                        prev_value = None
                    yield WatchEvent(
                        event.kv.key,
    ...
    async def txn(
        ...
        .. code-block:: python
            ...
            >>> await communicator.put(b'/tmp/successkey', b'1111')
            >>> def _txn_builder(action):
            ...     action.get(b'/tmp/successkey')
            ...
    ...
    async def txn_compare(
        ...
        .. code-block:: python
            ...
            >>> await communicator.put(b'/tmp/successkey', b'1111')
            >>> await communicator.put(b'/tmp/comparekey', b'asd')
            >>> await communicator.put(b'/tmp/comparekey2', b'asdg')
            >>> def _txn_builder(success, failure):
            ...     success.get(b'/tmp/successkey')
            ...
            >>> values = await communicator.txn_compare(
                    [
                        CompareKey('/tmp/comparekey').value == b'asd'],
                        CompareKey('/tmp/comparekey2').value > b'asdf'
                    ],
                    _txn_builder,
                )
            >>> print(values)  # [b'1111']
        ...
        values: list[TxnReturnType]
            ...
            If the call is `put` or `delete`, `None` will take that place.
        """
        txn = EtcdTransaction(self.channel)
...
class EtcdTransaction:
    ...
    def __init__(self, channel: Channel) -> None:
        ...
        self.success = EtcdTransactionAction()
        self.failure = EtcdTransactionAction()
...
class EtcdLockManager:
    ...
    async def __aenter__(self) -> None:
        ...
        if self.ttl is not None:
            communicator = EtcdCommunicator(self.channel)
